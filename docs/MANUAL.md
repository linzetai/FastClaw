...
```bash
git clone https://github.com/linzetai/XiaoLin.git
cd xiaolin
...
```json5
{
  "gateway": {
    ...
    "bind": "loopback",         // 绑定模式："loopback"（默认）/ "lan" / "custom"
    "customBindHost": null,     // 仅 bind="custom" 时有效
    "maxConnections": 0,        // 最大并发连接数（0 = 无限制）
    "corsOrigins": [],          // CORS 允许源，空数组 = 仅同源；["*"] = 允许全部
    "rateLimit": {
      "enabled": false,         // 是否启用速率限制
      "maxRequests": 100,       // 窗口内最大请求数
      "windowSecs": 60,         // 窗口时长（秒）
      "trustedProxies": []      // 信任的代理 IP（不计入限流）
    }
  }
}
```
...
**bind 模式：**
- `loopback` — 仅监听 `127.0.0.1`（默认，最安全）
- `lan` — 监听 `0.0.0.0`，允许局域网访问
- `custom` — 使用 `customBindHost` 指定的地址

> **安全提示**：当 `bind` 设置为 `lan` 或 `custom`（非 loopback 地址）时，必须配置 `security.apiKeys`，否则启动会被拒绝。
...
```json5
{
  "models": {
    "openai": {
      "provider": "openai_compatible",  // openai_compatible / anthropic（别名 providerType）
      "model": "gpt-4o",               // 模型名称（别名 defaultModel）
      "baseUrl": "https://api.openai.com/v1",
      "apiKey": null,                   // 可选，覆盖 credentials 中的配置
      "temperature": 0.0,
      "maxTokens": null,                // 最大输出 token 数
      "contextWindow": 128000,          // 模型上下文窗口大小
      "supportsVision": true,
      "supportsToolCalling": true,
      "supportsReasoning": false,
      "capabilities": {                 // 结构化能力声明（优先于 supports_* 布尔值）
        "input": ["text", "image"],     // text / image / audio / video
        "output": ["text", "tool_calls"] // text / tool_calls / reasoning
      },
...
```json5
{
  "session": {
    ...
    "dmScope": "main",                      // 会话作用域（见下方枚举）
    "reset": {                              // 会话自动重置策略
      "dailyHour": null,                    // 每天几点重置（0-23），null = 不重置
      "idleMinutes": null                   // 空闲多少分钟后重置，null = 不重置
    },
    "identityLinks": []                     // 跨渠道身份关联
  }
}
```

**dmScope 选项**（kebab-case 序列化）：
- `main` — 所有 DM 共享一个会话（默认，适合单用户场景）
- `per-peer` — 按发送者隔离会话（跨渠道）
...
```json5
{
  "memory": {
    "enabled": true,
    "dreamingIntervalSecs": 3600,        // 自动 dream cycle 间隔（秒），0 = 禁用
    "consolidationMinMessages": 6,       // 触发自动整合的最少非系统消息数
    "consolidationModel": null,          // 整合摘要使用的模型（null = 默认模型）
    "importance": {                      // 重要性评分权重（用于自动整合和记录）
      "weightLength": 0.15,
      "weightToolCalls": 0.25,
      "weightKeywords": 0.30,
      "weightDepth": 0.15,
      "weightCorrections": 0.15,
      "minThreshold": 0.3
    },
    "embedding": {
      "provider": "local",              // "local"（默认）/ "remote"
      "model": "sentence-transformers/all-MiniLM-L6-v2",
      "baseUrl": null,                  // remote 模式的 API URL
      "apiKey": null,                   // remote 模式的 API Key
      "dimensions": null                // 嵌入维度（null = 自动检测）
    }
  }
}
```
...
```json5
{
  "modelRouter": {
    "enabled": false,
    "strategy": "fallback",       // "fixed" / "cost_optimized" / "fallback"
    "dailyBudget": null,          // 每日预算上限（美元），null = 无限制
    "fallbackChain": []           // 回退模型链（用于 "fallback" 策略）
  }
}
```
...
**路由策略：**
- `fixed` — 始终使用固定模型
- `cost_optimized` — 根据成本选择最优模型
- `fallback` — 按 `fallbackChain` 顺序尝试，前一个失败时回退到下一个

### 3.8 LLM / Channel 插件配置

```json5
{
  "llmPlugins": {
    "enabled": true,              // 是否启用 LLM provider 插件系统
    "pluginsDir": null            // 插件目录路径（null = 默认）
  },
  "channelPlugins": {
    "enabled": true,              // 是否启用 Channel 插件系统
    "pluginsDir": null            // 插件目录路径（null = 默认）
  }
}
```

通过插件系统可扩展自定义 LLM 提供商和消息渠道，无需修改核心代码。
...
```json5
{
  "security": {
    ...
    "ssrfAllowedHosts": [],             // 绕过 SSRF 私有 IP 检查的主机名列表
    "dangerousOpsPolicy": "confirm",    // "deny" / "allow" / "confirm"（默认）
    "dangerousPatterns": [],            // 危险命令正则（默认覆盖 rm/chmod/dd 等）
    "sandboxEnabled": false,            // 是否启用 OS 级 Shell 沙箱
    "sandboxDenyGlobs": [],             // 沙箱内始终拒绝的路径 glob
    "execPolicyPath": null,             // exec-policy TOML 路径
    "guardianEnabled": false,           // 是否启用 Guardian LLM 审查
    "guardianModel": null,              // Guardian 使用的模型（默认 gpt-4o-mini）
    "guardianTimeoutSecs": null         // Guardian 审查超时（默认 60 秒）
  }
}
```

**dangerousOpsPolicy 选项：**
- `deny` — 直接拒绝所有危险操作
- `allow` — 允许执行（不推荐）
- `confirm` — 暂停并请求用户确认（默认）
...
{
  "paths": {
    "stateDir": null,       // 状态目录（默认 ~/.xiaolin 或 ~/.xiaolin-dev）
    "dbPath": null,         // 数据库文件路径
    "pluginsDir": null,     // 插件目录
    "extensionsDir": null,  // 扩展目录
    "skillsDir": null,      // Skills 目录
    "agentsDir": null       // Agent 配置目录
  }
}
```

所有路径字段默认为 `null`，系统会自动使用约定的默认路径。
...
{
  "skills": {
    "promptMode": "full",   // "full" / "compact" / "lazy"
    "allow": [],            // 允许的 Skill ID 列表（空 = 全部允许）
    "deny": []              // 拒绝的 Skill ID 列表
  }
}
```

**promptMode 选项：**
- `full` — 注入完整 SKILL.md 内容（最准确，最多 token）
- `compact` — 仅注入名称 + 单行描述（约节省 50% token）
- `lazy` — 注入最小列表，提供 `list_skills`/`read_skill` 工具按需加载（约节省 95% token）
...
{
  "webSearch": {
    "backend": "builtin",    // "tavily" / "searxng" / "builtin" / ""
    "apiKey": null,           // Tavily API Key（也检查 credentials.tavily.apiKey）
    "baseUrl": null,          // SearXNG 实例 URL
    "engines": ["google"]     // builtin 模式启用的搜索引擎
  }
}
```

**搜索引擎（builtin 模式）：** `google`、`baidu`、`bing`、`sogou`、`360`
...
```json5
{
  "promptRouter": {
    ...
    "profiles": {
      "coder": { "rolePromptId": "code-assistant" }
    },
    "rules": [
      { "profile": "coder", "keywords": ["代码", "编程", "bug"] }
    ]
  }
}
```

根据用户消息中的关键词自动切换 Prompt 配置。
...
{
  "evolution": {
    "skillExtractionIntervalSecs": 3600,    // Skill 提取扫描间隔（秒），0 = 禁用
    "skillMaintenanceIntervalSecs": 86400   // Skill 维护间隔（秒），0 = 禁用
  }
}
```
...
{
  "tracing": {
    "conversationTrace": false   // 是否记录每次聊天的完整 Trace
  }
}
```
...
| `strictIncludes` | `bool` | `$include` 文件缺失时是否报错（默认 `false`，仅警告） |
| `mcpServers` | `array` | 全局 MCP Server 配置（所有 Agent 共享） |
| `onboarding` | `object?` | 引导流程状态 |
...
连接地址：`ws://localhost:18789/ws`，协议为 `xiaolin-ws/2`，使用 JSON 文本帧。

**认证方式（三选一）：**

1. **Query 参数**：`ws://localhost:18789/ws?token=<api_key>`
2. **HTTP Header**（Upgrade 时）：`Authorization: Bearer <key>` 或 `X-API-Key: <key>`
3. **连接后认证**：先连接，再发送 `auth` method

```json
{"id": "1", "method": "auth", "params": {"token": "your-api-key"}}
```

**连接成功后立即收到 `connected` 消息：**
...
### 7.2 请求格式（JSON-RPC 风格）

所有客户端消息使用 `method` + `params` + `id` 格式：
...
服务端响应使用 `type` + `data` + `error` 格式，`id` 关联对应的请求：

```json
{
  "id": "req-1",
  "type": "turn_start",
  "data": {"model": "qwen3.5-plus", "sessionId": "..."}
}
```
...
聊天过程中，服务端按顺序推送以下事件（均携带请求的 `id`）：

**Turn 生命周期：**

| type | 说明 |
|------|------|
| `turn_start` | Turn 开始，包含模型名和 sessionId |
| `turn_end` | Turn 完成，包含 usage 统计 |
| `turn_aborted` | Turn 被中止（用户取消等） |

**内容流式：**

| type | 说明 |
|------|------|
| `content_delta` | 流式文本增量，包含 `delta` 字段 |
| `reasoning_delta` | 推理过程增量（支持推理的模型） |

**工具执行：**

| type | 说明 |
|------|------|
| `tool_executing` | 工具开始执行，包含工具名和参数 |
| `tool_result` | 工具执行完成，包含结果 |
...
| type | 说明 |
|------|------|
| `ask_question` | 请求用户输入（通过 `submit_tool_answer` 回复） |
| `approval_required` | 需要用户批准操作 |
| `approval_resolved` | 用户已处理审批 |
| `brief_message` | 简短提示消息 |
| `suggestions` | 建议列表 |
...
| type | 说明 |
|------|------|
| `context_warning` | 上下文接近上限警告 |
| `context_usage_update` | 上下文用量更新 |
...
| `sub_agent_delta` | Sub-agent 流式输出 |
| `sub_agent_tool_executing` / `sub_agent_tool_result` | Sub-agent 工具事件 |
| `sub_agent_complete` | Sub-agent 完成 |
...
|------|------|
| `error` | 致命错误 |
| `stream_error` | 流式过程中的错误 |
| `warning` | 非致命警告 |

### 7.5 完整对话示例

```
→ {"id":"1","method":"chat","params":{"messages":[{"role":"user","content":"你好"}],"stream":true}}
← {"id":"1","type":"turn_start","data":{"model":"qwen3.5-plus","sessionId":"sess_abc"}}
← {"id":"1","type":"content_delta","data":{"type":"content_delta","delta":{"text":"你好"}}}
← {"id":"1","type":"content_delta","data":{"type":"content_delta","delta":{"text":"！有什么可以帮你的？"}}}
← {"id":"1","type":"turn_end","data":{"type":"turn_end","summary":{"inputTokens":50,"outputTokens":20}}}
```
...
```json5
{
  "modelRouter": {
    "enabled": true,
    "strategy": "fallback",
    "fallbackChain": ["dashscope", "openai", "anthropic"],
    "dailyBudget": 10.0
  }
}
```
...
模型路由内置成本追踪，根据模型定义中的 `costPer1kInput` / `costPer1kOutput` 计算实际开销。设置 `dailyBudget` 可限制每日最大花费（美元）。
...
XiaoLin 支持两种 process-based 插件：LLM Provider 插件和 Channel 插件。插件以独立进程运行，通过 stdio JSON-RPC 与主进程通信。

### 14.1 LLM Provider 插件
...
```json5
{
  "llmPlugins": {
    "enabled": true,
    "pluginsDir": null    // 自定义插件目录（默认从 paths 推导）
  }
}
```
...
插件是可执行程序，通过 stdin/stdout 进行 JSON-RPC 通信。系统在启动时自动扫描插件目录并加载。
...
**核心对话**
- 多 Agent 流式聊天，通过 WebSocket 连接内嵌 Gateway
...
5. 前端通过 WebSocket 连接内嵌 Gateway 进行业务交互

如果 Gateway 启动失败，弹出系统通知告知用户。

### 22.5 通信架构

桌面端采用 **WebSocket + 少量 IPC** 的混合架构：

- **业务逻辑**（聊天、会话、Agent、配置、渠道、MCP、定时任务、通知等）全部通过 **WebSocket** 连接内嵌 Gateway，使用 `xiaolin-ws/2` 协议（见 §7）
- **本地操作**（文件读写、系统剪贴板等）通过 **Tauri IPC** 直接调用 Rust Commands

**Tauri IPC Commands（仅本地操作）：**

| 模块 | 命令 | 说明 |
|------|------|------|
| **Config** | `get_gateway_info` | 获取内嵌 Gateway 地址和端口 |
| **Agent** | `upload_agent_avatar` | 上传 Agent 头像（本地文件操作） |
| | `read_identity_files` | 读取身份文件（SOUL.md / USER.md / AGENTS.md） |
| **Session** | `export_session_content` | 导出会话内容到本地文件 |
| **Skill** | `upload_skill` | 上传自定义 Skill 文件 |
| **Migration** | `import_data` / `export_data` | 数据导入导出（ZIP 格式） |
| **Clipboard** | `clipboard_read_image` / `read_image_file` | 读取剪贴板或本地图片 |
...
│   │   ├── notification/       # 通知中心
│   │   └── onboarding/         # 首次使用引导
│   ├── lib/                    # 状态管理（Zustand stores）+ WebSocket 客户端
│   └── hooks/                  # React hooks
├── src-tauri/                  # Tauri Rust 后端
│   ├── src/
│   │   ├── lib.rs              # 应用入口、托盘、快捷键
│   │   ├── embedded.rs         # 内嵌 Gateway 启动/管理
│   │   └── commands/           # IPC Command 模块（本地操作）
