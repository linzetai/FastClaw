...
impl Tool for FeishuDriveTool {
    ...
    fn parameters_schema(&self) -> ToolParameterSchema {
        ...
        properties.insert("file_token".into(), serde_json::json!({"type": "string", "description": "File or folder token"}));
        ...
        properties.insert("name".into(), serde_json::json!({"type": "string", "description": "Folder name (for create_folder)"}));
        properties.insert("comment_id".into(), serde_json::json!({"type": "string", "description": "Comment ID (for reply_comment)"}));
        properties.insert("content".into(), serde_json::json!({"type": "string", "description": "Comment text content"}));
        properties.insert("page_size".into(), serde_json::json!({"type": "integer", "description": "Page size (1-100)"}));
    ...
    async fn execute(&self, arguments: &str) -> ToolResult {
        ...
        let result = match action {
            ...
            "info" => {
                ...
                self.client.user_post_json("/drive/v1/metas/batch_query", &body).await
            }
            "create_folder" => {
                ...
                self.client.user_post_json("/drive/v1/files/create_folder", &body).await
            }
            "move" => {
                let file_token = match args.get("file_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                ...
                let folder_token = match args.get("folder_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("folder_token (target) is required for move".to_string()),
                };
                ...
                self.client.user_post_json(&format!("/drive/v1/files/{file_token}/move"), &body).await
            }
            "delete" => {
                let file_token = match args.get("file_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
            ...
            "list_comments" => {
                let file_token = match args.get("file_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("file_token is required for list_comments".to_string()),
                };
                ...
                self.client.user_get_query(&format!("/drive/v1/files/{file_token}/comments"), &pairs).await
            }
            "add_comment" => {
                let file_token = match args.get("file_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("file_token is required for add_comment".to_string()),
                };
                ...
                self.client.user_post_json(&format!("/drive/v1/files/{file_token}/comments"), &body).await
            }
            "reply_comment" => {
                let file_token = match args.get("file_token").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("file_token is required for reply_comment".to_string()),
                };
                let comment_id = match args.get("comment_id").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("comment_id is required for reply_comment".to_string()),
                };
                let content = match args.get("content").and_then(|v| v.as_str()) {
                    Some(s) if !s.is_empty() => s,
                    _ => return ToolResult::err("content is required for reply_comment".to_string()),
                };
                let body = serde_json::json!({
                    "content": { "elements": [{ "type": "textRun", "textRun": { "text": content } }] }
                });
                self.client.user_post_json(&format!("/drive/v1/files/{file_token}/comments/{comment_id}/replies"), &body).await
            }
