...
use fastclaw_security::{FileSystemSandboxKind, FileSystemSandboxPolicy};
            ...
            if fs_policy.has_full_disk_write_access() {
                ...
                let writable_roots: Vec<PathBuf> = fs_policy
                    ...
                    .filter_map(|e| match &e.path {
                        fastclaw_security::FileSystemPath::Path { path } => {
...
mod tests {
    ...
    fn compatibility_from_fs_sandbox_with_writable() {
        use fastclaw_security::{FileSystemAccessMode, FileSystemPath, FileSystemSandboxEntry};
