...
use fastclaw_security::{FileSystemSandboxPolicy, NetworkSandboxPolicy};
...
/// - `FASTCLAW_SANDBOXED`: "1" (always set)
/// - `FASTCLAW_SANDBOX_LEVEL`: "off" | "standard" | "strict"
/// - `FASTCLAW_SANDBOX_FS_POLICY`: JSON-serialized filesystem sandbox policy
/// - `FASTCLAW_SANDBOX_NET_POLICY`: JSON-serialized network sandbox policy
pub fn transform_with_level(
    ...
    env.insert("FASTCLAW_SANDBOX_LEVEL".to_string(), level_str.to_string());

    if let Ok(fs_json) = serde_json::to_string(fs_policy) {
        env.insert("FASTCLAW_SANDBOX_FS_POLICY".to_string(), fs_json);
    }
    if let Ok(net_json) = serde_json::to_string(&net_policy) {
        env.insert("FASTCLAW_SANDBOX_NET_POLICY".to_string(), net_json);
    }
...
mod tests {
    ...
    fn windows_transform_sets_env_markers() {
        ...
        assert_eq!(cmd.env.get("FASTCLAW_SANDBOXED").unwrap(), "1");
        assert!(cmd.env.contains_key("FASTCLAW_SANDBOX_FS_POLICY"));
        assert!(cmd.env.contains_key("FASTCLAW_SANDBOX_NET_POLICY"));
    }
