...
const MISSING_BWRAP_WARNING: &str = concat!(
    "FastClaw could not find bubblewrap on PATH. ",
    "Install bubblewrap with your OS package manager. ",
    "FastClaw's Linux sandbox requires bubblewrap for full isolation.",
);

const USER_NAMESPACE_WARNING: &str =
    "FastClaw's Linux sandbox uses bubblewrap and needs access to create user namespaces.";

pub(crate) const WSL1_BWRAP_WARNING: &str = concat!(
    "FastClaw's Linux sandbox uses bubblewrap, which is not supported on WSL1 ",
