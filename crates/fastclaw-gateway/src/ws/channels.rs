...
pub async fn handle_wechat_login(
    ...
    let existing_tokens: Vec<String> = fastclaw_wechat::auth::credential::list_credentials()
        .into_iter()
        .map(|(_, cred)| cred.token)
        .collect();

    match fastclaw_wechat::auth::qr_login::start_login(&existing_tokens).await {
        Ok(session) => {
...
pub async fn handle_wechat_poll(
    ...
    let status = fastclaw_wechat::auth::qr_login::poll_login(&mut entry).await;
    use fastclaw_wechat::auth::qr_login::LoginStatus;
    ...
    match status {
        ...
        LoginStatus::Confirmed {
            ...
            let normalized_id =
                fastclaw_wechat::auth::credential::normalize_account_id(&account_id);
            let cred = fastclaw_wechat::auth::credential::WechatCredential {
                token: bot_token,
                base_url,
                user_id,
                cdn_base_url: None,
            ...
            let _ = fastclaw_wechat::auth::credential::save_credential(&normalized_id, &cred);
        ...
        LoginStatus::AlreadyConnected => {
            ...
            send_resp(
                ...
                &WsResponse {
                    id: req_id,
                    msg_type: "channels.wechat_poll".into(),
                    data: Some(json!({"status": "already_connected", "message": "已连接过此 FastClaw"})),
                    error: None,
                },
            )
            .await;
        ...
        LoginStatus::Expired => {
            let existing_tokens: Vec<String> =
                fastclaw_wechat::auth::credential::list_credentials()
                    .into_iter()
                    .map(|(_, c)| c.token)
                    .collect();
            let refresh_result =
                fastclaw_wechat::auth::qr_login::refresh_qr(&mut entry, &existing_tokens).await;
...
pub async fn handle_wechat_verify(
    ...
    if let Some(mut entry) = state.ext.wechat_login_sessions.get_mut(session_key) {
        fastclaw_wechat::auth::qr_login::set_verify_code(&mut entry, code);
        send_resp(
            sender,
            &WsResponse {
                id: req_id,
...
async fn restart_wechat_channel(state: &AppState) {
    ...
    let plugin = std::sync::Arc::new(fastclaw_wechat::WechatPlugin::new(wechat_config));
