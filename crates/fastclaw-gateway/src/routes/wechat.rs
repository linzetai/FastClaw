...
use fastclaw_core::config_access::{persist_config_key, set_nested_key};
use fastclaw_wechat::auth::credential;
use fastclaw_wechat::auth::qr_login;
...
pub async fn reload_channel(State(state): State<AppState>) -> impl IntoResponse {
    ...
    match fastclaw_core::config::load_config(mode) {
        Ok(fresh) => {
