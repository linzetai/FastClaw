...
const PROXY_SOCKET_DIR_PREFIX: &str = "fastclaw-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(fastclaw_home) = std::env::var_os("FASTCLAW_HOME") {
        let candidate = PathBuf::from(fastclaw_home).join("tmp");
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("fastclaw-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("fastclaw-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("fastclaw-linux-sandbox-proxy-x"),
            None
        );
