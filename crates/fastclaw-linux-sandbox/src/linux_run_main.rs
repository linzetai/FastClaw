...
use fastclaw_security::FileSystemSandboxPolicy;
...
pub fn run_main() -> Result<()> {
    ...
    if policy.use_bwrap {
        ...
        let fs_policy = policy
            ...
            .clone()
            .unwrap_or_else(|| {
                let writable_abs: Vec<fastclaw_path::AbsolutePathBuf> = policy
                    .writable_roots
                    .iter()
                    .filter_map(|p| fastclaw_path::AbsolutePathBuf::from_absolute_path(p).ok())
                    .collect();
...
mod tests {
    ...
    fn parse_args_unknown_flag() {
        let args = vec!["fastclaw-linux-sandbox".into(), "--bogus".into()];
