...
const PROTECTED_METADATA_FASTCLAW_PATH_NAME: &str = ".fastclaw";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_FASTCLAW_PATH_NAME,
];

const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "fastclaw-project-roots://";
...
/// FastClaw manages the sandbox itself, an external caller does, or
/// sandboxing is disabled entirely.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SandboxEnforcement {
    /// FastClaw owns sandbox construction (Landlock, Seatbelt, etc.).
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where FastClaw manages the sandbox.
...
pub enum PermissionProfile {
    /// FastClaw owns sandbox construction for this profile.
    Managed {
...
pub struct WritableRoot {
    ...
    /// Workspace metadata path names (e.g. `.git`, `.fastclaw`) that must not
...
fn default_read_only_subpaths_for_writable_root(
    ...
    let top_level_fastclaw = writable_root.join(PROTECTED_METADATA_FASTCLAW_PATH_NAME);
    if top_level_fastclaw.as_path().is_dir() {
        subpaths.push(top_level_fastclaw);
    }
    subpaths
}
...
mod tests {
    use super::*;
    use fastclaw_path::test_support::{test_path_buf, PathBufExt};
    ...
    fn file_system_sandbox_policy_json_roundtrip() {
        ...
        let _guard = fastclaw_path::AbsolutePathBufGuard::new(Path::new("/"));
    ...
    fn is_protected_metadata_name_checks() {
        ...
        assert!(is_protected_metadata_name(OsStr::new(".fastclaw")));
