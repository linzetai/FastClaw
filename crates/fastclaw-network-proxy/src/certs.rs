...
//! `$HOME/.fastclaw/proxy/`. Each target host gets a dynamically
...
fn managed_ca_paths() -> Result<(PathBuf, PathBuf)> {
    ...
    let proxy_dir = home.join(".fastclaw").join(MANAGED_MITM_CA_DIR);
    Ok((
...
fn generate_ca() -> Result<(String, String)> {
    ...
    dn.push(DnType::CommonName, "FastClaw Network Proxy MITM CA");
