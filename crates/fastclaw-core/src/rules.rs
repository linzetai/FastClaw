...
/// A project-level rule loaded from `.fastclaw/rules/*.md`.
...
/// Load all rules from a directory (`.fastclaw/rules/` or `.cursor/rules/`).
...
/// Load rules from the workspace root, scanning both `.fastclaw/rules/` and `.cursor/rules/`.
pub fn load_project_rules(workspace_root: &Path) -> RuleRegistry {
    let mut registry = RuleRegistry::new();
    let fastclaw_rules = load_rules_from_dir(&workspace_root.join(".fastclaw/rules"));
    let cursor_rules = load_rules_from_dir(&workspace_root.join(".cursor/rules"));
    registry.merge_from(cursor_rules);
    registry.merge_from(fastclaw_rules);
    registry
}
