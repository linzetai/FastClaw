use fastclaw_core::migration::{ExportOptions, ImportOptions, export_data, import_data};
...
async fn test_migration_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 序列化数据
    let serialized_data = fastclaw_core::migration::serialize_migration_data(&migration_data)?;
    println!("✓ Data serialized to {} bytes", serialized_data.len());

    // 反序列化数据
    let deserialized_data = fastclaw_core::migration::deserialize_migration_data(&serialized_data)?;
